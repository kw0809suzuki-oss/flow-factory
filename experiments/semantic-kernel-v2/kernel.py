...
class ExperimentResult:
    ...
    local_behavior_change: str = "unknown"
    flow_change: str = "unknown"
    terminal_changed: str = "unknown"
    terminal_self_effect: Optional[float] = None
    margin_effect: Optional[float] = None
    win_loss_effect: str = "unknown"
    fresh_verification: str = "not_run"
...
def evaluate(
    ...
    rules: Sequence[Rule],
) -> tuple[tuple[Rule, ...], StepTrace]:
    """Evaluate every trigger against exactly one immutable snapshot."""
...
def _rules_conflict(a: Rule, b: Rule) -> bool:
    return (
        a.exclusive
        or b.exclusive
        or b.id in a.conflicts_with
        or a.id in b.conflicts_with
    )


def resolve(
    snapshot: StateSnapshot,
    eligible_rules: Sequence[Rule],
    trace: StepTrace,
) -> tuple[tuple[Rule, ...], StepTrace]:
    """Resolve only rule-level semantics. Never inspect resource sufficiency here."""
    del snapshot
    ...
    remaining = [r for r in eligible_rules if r.id not in rejections]
    ranked = sorted(remaining, key=lambda r: (-r.priority, r.id))
    selected: list[Rule] = []
    ...
    return selected_tuple, next_trace
...
def _order_actions(actions: Sequence[ActionIntent]) -> tuple[ActionIntent, ...] | None:
    ...
    while remaining:
        ready = [
            ...
            if deps[action_id].isdisjoint(remaining)
        ]
        if not ready:
            return None
...
def plan(
    ...
    trace: StepTrace,
) -> tuple[Optional[ActionBundle], StepTrace]:
    """
    Validate and order selected actions.

    Crucially, an invalid bundle does not rewrite selected_rule_ids.
    """
    ...
    if ordered is None:
        return None, replace(
            trace,
            plan_status="invalid",
            plan_reason="invalid_action_dependency_graph",
            planned_action_ids=(),
            executed_action_ids=(),
        )
...
def apply(
    ...
    trace: StepTrace,
) -> tuple[StateSnapshot, StepTrace]:
    """Apply an already validated bundle. No trigger evaluation or rule resolution is allowed."""
